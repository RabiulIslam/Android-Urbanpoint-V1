...
public class Profile_WebHit_Get_BriteVerifyEmail {
    private static final String TAG = "UPDATEPROFLE";
    private AsyncHttpClient mClient = new AsyncHttpClient();
    public static ResponseModel responseModel = null;


    public void checkValidation(final Context _context, final ProfileFragment profileFragment,
                                final String _email) {
...
//        mClient.addHeader("Authorization", "Bearer 73c4e5a59721c0b3a40b97a32dec873d2e09fd97");
        ...
        mClient.get(myUrl, new AsyncHttpResponseHandler() {
                    ...
                    public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                        ...
                        try {
                            ...
                            if (responseModel.getStatus().equals(Constants.DEFAULT_VALUES.VALID)) {
                                profileFragment.showResultEmailValidation(true, "");
                            } else if (responseModel.getStatus().equals(Constants.DEFAULT_VALUES.UNKNOWN)) {
                                profileFragment.showResultEmailValidation(true, "");
                            } else {
                                profileFragment.showResultEmailValidation(false, responseModel.getStatus());
                            }
                        } catch (Exception ex) {
                            ex.printStackTrace();
                            profileFragment.showResultEmailValidation(false, ex.toString());
                        }
                    }
                    ...
                    public void onFailure(int statusCode, Header[] headers, byte[] responseBody, Throwable
                            error) {
                        profileFragment.showResultEmailValidation(false, error.getMessage());
