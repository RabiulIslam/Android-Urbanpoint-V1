...
public class HoloCircleSeekBar extends View {
    ...
    private Typeface novaRegular = Typeface.createFromAsset(getContext().getAssets(), "fonts/proxima_nova_alt_regular.ttf");
    ...
    //10 is added in angleValue as we required to start from 10
    private void setTextFromAngle(int angleValue) {
        if (angleValue < 60) {
            this.text = String.valueOf(angleValue + 10);
        } else
            this.text = "70+";
    }
...
//        String wheel_color_attr = getContext().getResources().getColor(R.color.transparent);
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // mCircleTextColor);
        textPaint.setTypeface(novaRegular);
        if (show_text)
            canvas.drawText(
                    text,
